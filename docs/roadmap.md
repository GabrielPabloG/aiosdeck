...
| **E1.6** | **Execution Reliability: Timeouts + Process Supervision** — per-task-type timeouts (e.g. `quick_test 60s`, `unit_test 120s`, `build 300s`, `browser_qa 300s`, `large_refactor 600s`) replacing the single hardcoded 600 s; step-level timeouts (`shell_command`, `model_turn`, `process_start`); timeout → cancel → kill child processes → collect diagnostics → `TIMEOUT` with last action recorded. E1.6 step-level timeout design should account for the outcome of the OpenCode step budget enforcement investigation. | 🟦 | [#99](https://github.com/GabrielPabloG/aiosdeck/issues/99)–#102, [#152](https://github.com/GabrielPabloG/aiosdeck/issues/152) |
